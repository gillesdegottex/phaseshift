...
#include <phaseshift/audio_block/ola_decoupled.h>
#include <phaseshift/audio_block/tinywavfile.h>
#include <phaseshift/sigproc/sigproc.h>

void phaseshift::ola_decoupled::proc_frame(const phaseshift::vector<float>& in, phaseshift::vector<float>* pout, const phaseshift::ola_decoupled::proc_status& status) {
    (void)status;
...
phaseshift::ola_decoupled::ola_decoupled() {
}

phaseshift::ola_decoupled::~ola_decoupled() {
}

int phaseshift::ola_decoupled::output_one_frame(phaseshift::ringbuffer<float>* pout, int nb_samples_to_output) {
    assert(pout != nullptr && "phaseshift::ola_decoupled::output_one_frame: The output buffer is nullptr.");
    ...
    if (pout->size() + nb_samples_to_output_remains > pout->size_max()) {
        ...
        assert(pout->size() + nb_samples_to_output_remains <= pout->size_max() && 
               "phaseshift::ola_decoupled::output_one_frame: There is not enough space in the output buffer");
    } else {
...
int phaseshift::ola_decoupled::process(const phaseshift::ringbuffer<float>& in, phaseshift::ringbuffer<float>* pout) {
    proc_time_start();
    ...
    while (in_n < in.size()) {
        ...
        while (m_frame_rolling.size() == winlen()) {
            ...
            assert(m_frame_input.size() > 0 && "phaseshift::ola_decoupled::process: The input frame is empty.");

            // Update input cursor for status
            m_input_win_center_idx = m_input_win_center_idx_next;
            assert(m_input_win_center_idx >= 0 && "phaseshift::ola_decoupled::process: The input window center index is negative.");
...
int phaseshift::ola_decoupled::flush(int chunk_size_max, phaseshift::ringbuffer<float>* pout) {
    proc_time_start();
...
int phaseshift::ola_decoupled::fetch(phaseshift::ringbuffer<float>* pout, int chunk_size_max) {
    ...
    assert(pout->size() + chunk_size <= pout->size_max() && "phaseshift::ola_decoupled::fetch: There is not enough space in the output buffer");
...
void phaseshift::ola_decoupled::process_offline(const phaseshift::ringbuffer<float>& in, phaseshift::ringbuffer<float>* pout) {
    process(in, pout);
    flush(-1, pout);
}

void phaseshift::ola_decoupled::process_offline(const phaseshift::ringbuffer<float>& in, phaseshift::ringbuffer<float>* pout, int chunk_size) {
...
void phaseshift::ola_decoupled::process_realtime(const phaseshift::ringbuffer<float>& in, phaseshift::ringbuffer<float>* pout) {
...
void phaseshift::ola_decoupled::reset() {
    ...
    m_status.reset();
    phaseshift::ola_decoupled::m_failure_status.reset();
...
phaseshift::ola_decoupled* phaseshift::ola_decoupled_builder::build(phaseshift::ola_decoupled* pab) {
    build_time_start();
    ...
    assert((m_winlen > 0) && "phaseshift::ola_decoupled_builder::build: winlen has to be >0");
    assert((m_winlen > m_timestep) && "phaseshift::ola_decoupled_builder::build: time step has to be smaller or equal to window's length");
    ...
    pab->phaseshift::ola_decoupled::reset();
...
void phaseshift::dev::audio_block_ola_decoupled_test(phaseshift::ola_decoupled* pab, int chunk_size, float resynthesis_threshold, int options) {

    float duration_s = 3.0f;

    // Static tests
    phaseshift::dev::test_require(pab->fs() > 0.0f, "audio_block_ola_decoupled_test: fs() <= 0.0f");
    phaseshift::dev::test_require(pab->latency() >= 0, "audio_block_ola_decoupled_test: latency() < 0");
            ...
            for (int iter=1; iter <= 3; ++iter) {
                ...
                if (mode == int(mode_offline)) {
                    ...
                    while (signal_out.size() < signal_in.size()) {
                        ...
                        phaseshift::dev::test_require(chunk_in.size() == signal_out_size_after-signal_out_size_before, 
                            "audio_block_ola_decoupled_test: chunk_in.size() != signal_out_size_after-signal_out_size_before");
                    }
                }

                // Verify
                phaseshift::dev::test_require(signal_out.size() > 0, "audio_block_ola_decoupled_test: signal_out.size() == 0");
                phaseshift::dev::test_require(signal_out.size() == signal_in.size(), "audio_block_ola_decoupled_test: signal_out.size() != signal_in.size()");
                ...
                if ((mode == int(mode_offline)) || (mode == int(mode_streaming))) {
                    phaseshift::dev::test_require(phaseshift::dev::signals_equal_strictly(signal_in, signal_out, resynthesis_threshold), 
                        "audio_block_ola_decoupled_test: signals_equal_strictly() failed");

                } else if (mode == int(mode_realtime)) {
                    phaseshift::dev::test_require(pab->stat_realtime_out_size_min() < chunk_size, 
                        "audio_block_ola_decoupled_test: stat_realtime_out_size_min() >= chunk_size");
                    ...
                    if (synth == synth_click) {
                        if (options & option_test_latency_decoupled) {
                            ...
                            phaseshift::dev::test_require(measured_latency == pab->latency(), 
                                "audio_block_ola_decoupled_test: measured_latency != latency()");
                        }
                    }
                }
...
void phaseshift::dev::audio_block_ola_decoupled_builder_test_singlethread() {
    ...
    auto pbuilder = new phaseshift::ola_decoupled_builder();
    ...
    for (const auto& [fs, timestep, winlen, chunk_size] : test_combinations) {
        ...
        phaseshift::dev::audio_block_ola_decoupled_test(pab, chunk_size);
...
void phaseshift::dev::audio_block_ola_decoupled_builder_test(int nb_threads) {
    phaseshift::dev::audio_block_builder_test(phaseshift::dev::audio_block_ola_decoupled_builder_test_singlethread, 1);
}
